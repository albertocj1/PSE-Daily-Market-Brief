...
2. **News:** reads four feeds (BusinessWorld, Philstar Business, and two Google News queries: one for the PSEi, one for BSP/peso/inflation), keeps the last 30 hours, removes duplicates and social-media posts, and shows the original publisher for each headline.
...
- **Clean, honest output.** Google News titles are split into headline and publisher, social-media posts are dropped, long titles are capped, yields are shown in basis points, and the message says so when watchlist quotes are incomplete instead of silently omitting them.
...
| News feeds | The RSS nodes (keep names in sync with `SOURCES` in `Build LLM Input`) |
...
- **News coverage is limited** to four feeds and a 30-hour window.
...
MIT. Add a `LICENSE` file, or change this line to the license you prefer.
